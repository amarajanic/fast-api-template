...
async def create_blog(request:BlogInsert, db: Session):
    new_blog = DbBlog(title=request.title, body=request.body, user_id=request.user_id, theme_id= request.theme_id)
    db.add(new_blog)
    db.commit()
    db.refresh(new_blog)
    return new_blog
...
async def get_blogs(db: Session, search: str, order_by: OrderBy, skip: int = 0, limit: int = 100):
    order_dict = {"OrderBy.az": asc(DbBlog.title), "OrderBy.za": desc(DbBlog.title)}
