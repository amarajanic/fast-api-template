...
async def registrate_user(request: UserRegistrate, db: Session):
    if request.password != request.confirm_password:
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE,
                            detail=f"Your password and confirmation password do not match")
    ...
    new_user = DbUser(name=f"{request.first_name} {request.last_name}",
                      email=request.email, password=hashedPassword, role_id=1)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return new_user
...
async def forgot_password(email: str, db: Session):
    ...
    if dbUser:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"User with mail {email} doesn't exist.")
